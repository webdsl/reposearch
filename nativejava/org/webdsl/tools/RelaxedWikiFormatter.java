...
public final class RelaxedWikiFormatter {
  private static final Pattern links = Pattern
      .compile("\\[\\[(\\w+)(\\(([^\\)]*)\\))?(\\|([^\\]]+))?\\]\\]");
  private static final Pattern verbatim = Pattern.compile(
      "<verbatim>(.+?)</verbatim>", Pattern.DOTALL | Pattern.MULTILINE);

  public static String wikiFormat(String text) {
    // return org.jsoup.Jsoup.clean(
    return wikiFormat(text, utils.ThreadLocalPage.get().getAbsoluteLocation()
